...
The primary destination is named **Power Rankings**, never Teams. It is a league-wide current-year viability product, not a duplicate draft board. Its team deep dives must focus on 2026 competitiveness, scoring history, optimal-lineup strength, positional construction, depth, concentration and player-volatility proxies, with dynasty quality and three-year runway shown separately. Draft and roster opinions should always reflect the actual 1QB, half-PPR, no-TE-premium, three-FLEX league rules.

Draft grades and power-ranking grades answer different questions and must never share a detail screen. Draft-cycle grades are computed—not manually assigned—from 60% pick execution, 30% capital management and 10% roster fit. Individual pick grades must use the same nonlinear expert/market value-ratio curves as the aggregate execution grade, and the UI must explain that high-value early selections carry more weight than late picks.
